  ...
  try {
    connection.query(selectquery, function (error, result) {
      if (error) {
        ...
        var Insertquery = `insert into cash_book_total_val(month,financial_year,opening_bal)values('${M}','${Y}',${opening_bal})`;
...
app.post("/submit_gras", urlencodedParser, function (req, res) {
  ...
  if (req.session.user_type == "ABC") {
    var query = `insert into approved_gras(gras_receipt_no,particular,month,financial_year,budget_code,district,amount,insert_by,insert_ip,insert_dt)values('${gras_receipt}', '${particular}','${month}','${financial_year}', '${budget_code}', '${district}', '${amount}','${insert_by}','${user_ip}','${dt}');`;
    connection.query(query, function (error, results) {
      if (error) {
    ...
    connection.query(query, function (error, results) {
      ...
      if (error) {
        ...
        if (tablename == "approved_remittance") {
          ...
          insert_del = `insert into ${tablename1}(letter_no,so2_no,particular,sales_office,date,budget_code,payment_method,cheque_no,month,financial_year,amount,insert_ip,insert_by,insert_dt,update_ip,update_by,update_dt,delete_dt,delete_ip,delete_by)values('${results.letter}','${results.so2}','${results.particular}','${results.sales_office}','${results.date}','${results.budget_code}', '${results.payment}', '${results.cheque_no}','${results.month}','${results.financial_year}','${results.amount}','${results.insert_ip}','${results.insert_by}','${results.insert_dt}',
          '${results.update_ip}','${results.update_by}','${results.update_dt}','${date1}','${ip}','${req.session.username}'
                          );`;
        } else if (tablename == "approved_gras") {
          tablename1 = "deleted_gras";
          insert_del = `insert into ${tablename1}(gras_receipt_no,particular,date,month,financial_year,type_of_transaction,budget_code,district,amount,insert_ip,insert_by,insert_dt,update_ip,update_by,update_dt,delete_ip,delete_dt,delete_by)values('${results.gras_receipt}','${results.particular}','${results.date}', '${results.month}','${results.financial_year}','${results.type_of_transation}', '${results.budget_code}',  '${results.district}', '${results.amount}','${results.insert_ip}','${results.insert_by}','${results.insert_dt}',
          ...
          insert_del = `insert into ${tablename1}(particular,instrument,date,budget_code,payment_method,month,financial_year,amount,insert_ip,insert_by,insert_dt,update_ip,update_by,update_dt,delete_ip,delete_by,delete_dt)values('${results.particular}','${results.instrument}','${results.date}','${results.budget_code}','${results.payment}','${results.month}','${results.financial_year}','${results.amount}','${results.insert_ip}','${results.insert_by}','${results.insert_dt}',
...
app.post("/insert_opening_balance", urlencodedParser, (req, res) => {
  ...
  var query = `insert into cash_book_total_val values('${month}','${financial_year}','0','0','0','${opening_balance}','0')`;
  connection.query(query, function (error, result) {
    if (error) {
      res.send("Unsuccessful");
      console.log(error);
